        ...
        bootstrap: async ({ symbol, pair, interval, signal } = {}) => {
            ...
            try {
                const [depthSnapshot, contractKlines] = await Promise.all([
                    weightedGet(FUTURES_PRODUCTION_WORKSTATION_WEIGHTS.DEPTH_1000, FUTURES_PRODUCTION_WORKSTATION_ROUTES.DEPTH, { symbol, limit: String(FUTURES_PRODUCTION_WORKSTATION_REQUEST_LIMITS.DEPTH) }, FUTURES_WORKSTATION_BODY_LIMITS.DEPTH, batchController.signal, backendProxy),
                    weightedGet(FUTURES_PRODUCTION_WORKSTATION_WEIGHTS.KLINES_100, FUTURES_PRODUCTION_WORKSTATION_ROUTES.KLINES, { symbol, interval, limit: String(FUTURES_PRODUCTION_WORKSTATION_REQUEST_LIMITS.KLINES) }, FUTURES_WORKSTATION_BODY_LIMITS.KLINES, batchController.signal, backendProxy),
                ]);
                const [markKlines, indexKlines] = await Promise.all([
                    weightedGet(FUTURES_PRODUCTION_WORKSTATION_WEIGHTS.MARK_KLINES_100, FUTURES_PRODUCTION_WORKSTATION_ROUTES.MARK_KLINES, { symbol, interval, limit: String(FUTURES_PRODUCTION_WORKSTATION_REQUEST_LIMITS.KLINES) }, FUTURES_WORKSTATION_BODY_LIMITS.KLINES, batchController.signal, backendProxy),
                    weightedGet(FUTURES_PRODUCTION_WORKSTATION_WEIGHTS.INDEX_KLINES_100, FUTURES_PRODUCTION_WORKSTATION_ROUTES.INDEX_KLINES, { pair, interval, limit: String(FUTURES_PRODUCTION_WORKSTATION_REQUEST_LIMITS.KLINES) }, FUTURES_WORKSTATION_BODY_LIMITS.KLINES, batchController.signal, backendProxy),
                ]);
                const [premiumIndex, ticker] = await Promise.all([
                    weightedGet(FUTURES_PRODUCTION_WORKSTATION_WEIGHTS.PREMIUM_INDEX_SYMBOL, FUTURES_PRODUCTION_WORKSTATION_ROUTES.PREMIUM_INDEX, { symbol }, FUTURES_WORKSTATION_BODY_LIMITS.HEADER, batchController.signal, backendProxy),
                    weightedGet(FUTURES_PRODUCTION_WORKSTATION_WEIGHTS.TICKER_SYMBOL, FUTURES_PRODUCTION_WORKSTATION_ROUTES.TICKER, { symbol }, FUTURES_WORKSTATION_BODY_LIMITS.HEADER, batchController.signal, backendProxy),
                ]);
