...
const moduleMocks = vi.hoisted(() => {
    ...
    const reset = () => {
        ...
        state.spotPrivateSockets = [];
        state.autoSpotPrivateAck = true;
    ...
    return {
        ...
        makeSocket,
        reset,
        setSpotPrivateAutoAck: value => { state.autoSpotPrivateAck = value; },
        createSpotPrivateSocket: () => {
            const socket = makeSocket();
            socket.terminate = vi.fn(() => socket.handlers.close?.());
            socket.send = vi.fn((text, callback) => {
                callback?.();
                if (state.autoSpotPrivateAck) {
                    const frame = JSON.parse(text);
                    Promise.resolve().then(() => socket.handlers.message?.(JSON.stringify({
                        id: frame.id, status: 200, result: { subscriptionId: 0 },
                    }), false));
                }
            });
            state.spotPrivateSockets.push(socket);
            Promise.resolve().then(() => socket.handlers.open?.());
            return socket;
        },
        ...
        get spotPrivateSockets() { return state.spotPrivateSockets; },
...
vi.mock('ws', () => ({
    default: vi.fn(function MockExchangeSocket(url) {
        if (url.startsWith('wss://ws-api.binance.com')) return moduleMocks.createSpotPrivateSocket();
...
describe('setupBinanceConnection user-data orchestration', () => {
    ...
    it('discards a queued position mark before acknowledging a switch away from Futures', async () => {
        ...
        expect(delivered.filter(payload => payload.type === 'futures_position_marks'))
            .toHaveLength(1);
        expect(delivered.filter(payload => payload.type === 'market_activation').at(-1)).toMatchObject({
    ...
    describe('Spot WebSocket API private subscription ownership', () => {
        const privateFrame = (socket, event, subscriptionId = 0) => socket.handlers.message(
            JSON.stringify({ subscriptionId, event }), false,
        );
        const place = (clientOrderId) => moduleMocks.rendererHandlers.message({
            type: 'utf8', utf8Data: JSON.stringify({
                action: 'trade.placeOrder', version: 1, marketType: 'spot',
                clientOrderId, symbol: 'BTCUSDT', side: 'BUY', orderType: 'LIMIT',
                timeInForce: 'GTC', price: '50000', quantity: '0.001',
            }),
        });
        ...
        it('uses no retired REST listenKey, catches up balances/orders and publishes identity-scoped health', async () => {
            ...
            expect(JSON.parse(socket.send.mock.calls[0][0])).toMatchObject({
                method: 'userDataStream.subscribe.signature', params: { apiKey: 'test-api-key' },
            });
            expect(moduleMocks.sendRequest.mock.calls.some(([path]) => path === '/api/v3/userDataStream')).toBe(false);
            expect(moduleMocks.spotClient.restAPI.getAccount).toHaveBeenCalled();
            expect(moduleMocks.spotClient.restAPI.getOpenOrders).toHaveBeenCalled();
            privateFrame(socket, { e: 'executionReport', s: 'BTCUSDT', i: 91, X: 'NEW', S: 'BUY' });
            expect(emitted().some(frame => frame.execution_update?.orderId === 91)).toBe(true);
            privateFrame(socket, { e: 'outboundAccountPosition', B: [{ a: 'USDT', f: '9', l: '1' }] });
            expect(emitted().some(frame => frame.balance_update?.B?.[0]?.f === '9')).toBe(true);
            moduleMocks.rendererConnection.sendUTF.mockClear();
            privateFrame(socket, { e: 'executionReport', s: 'BTCUSDT', i: 92, X: 'NEW' }, 1);
            expect(emitted()).toEqual([]);
        });

        it('refuses placement before acknowledgement, allows cancellation and never replays the refused order', async () => {
            moduleMocks.setSpotPrivateAutoAck(false);
            moduleMocks.spotClient.restAPI.deleteOrder = vi.fn().mockResolvedValue({
                status: 200, data: async () => ({ symbol: 'BTCUSDT', orderId: 1, status: 'CANCELED' }),
            });
            await connectRenderer('spot');
            await place('private-not-ready');
            expect(moduleMocks.spotClient.restAPI.newOrder).not.toHaveBeenCalled();
            expect(emitted().some(frame => frame.command_rejected?.code === 'SPOT_PRIVATE_STREAM_UNAVAILABLE')).toBe(true);
            await moduleMocks.rendererHandlers.message({
                type: 'utf8', utf8Data: JSON.stringify({
                    action: 'trade.cancelOrder', version: 1, marketType: 'spot',
                    clientOrderId: 'cancel-while-unconfirmed', symbol: 'BTCUSDT', orderId: '1',
                }),
            });
            expect(moduleMocks.spotClient.restAPI.deleteOrder).toHaveBeenCalledOnce();
            const socket = moduleMocks.spotPrivateSockets[0];
            socket.handlers.message(JSON.stringify({
                id: JSON.parse(socket.send.mock.calls[0][0]).id, status: 200, result: { subscriptionId: 0 },
            }));
            await vi.advanceTimersByTimeAsync(2_000);
            expect(moduleMocks.spotClient.restAPI.newOrder).not.toHaveBeenCalled();
        ...
        it('does not let a held balance fallback undo a newer private event, and drains one trailing read', async () => {
            ...
            moduleMocks.spotClient.restAPI.getAccount.mockReset()
            ...
            privateFrame(socket, { e: 'balanceUpdate', a: 'USDT', d: '1' });
            await vi.advanceTimersByTimeAsync(500);
            expect(moduleMocks.spotClient.restAPI.getAccount).toHaveBeenCalledOnce();
            privateFrame(socket, { e: 'outboundAccountPosition', B: [{ a: 'USDT', f: '30', l: '0' }] });
            privateFrame(socket, { e: 'balanceUpdate', a: 'USDT', d: '2' });
            release({ balances: [{ asset: 'USDT', free: '10', locked: '0' }] });
            await vi.advanceTimersByTimeAsync(2_000);
            expect(moduleMocks.spotClient.restAPI.getAccount).toHaveBeenCalledTimes(2);
            const balances = emitted().filter(frame => frame.balances).map(frame => frame.balances.USDT?.available);
            expect(balances).toEqual(['30']);
        });

        it('does not publish a held balance response after its final consumer leaves', async () => {
            await connectRenderer('spot');
            let release;
            const held = new Promise(resolve => { release = resolve; });
            moduleMocks.spotClient.restAPI.getAccount.mockImplementationOnce(async () => ({ status: 200, data: () => held }));
            privateFrame(moduleMocks.spotPrivateSockets[0], { e: 'balanceUpdate', a: 'USDT', d: '1' });
            await vi.advanceTimersByTimeAsync(500);
            moduleMocks.rendererHandlers.close();
            moduleMocks.rendererConnection.sendUTF.mockClear();
        ...
        it('withdraws readiness, reconnects once and ignores late old-socket events', async () => {
            await connectRenderer('spot');
            const old = moduleMocks.spotPrivateSockets[0];
            old.handlers.close();
            await place('private-gap');
            expect(moduleMocks.spotClient.restAPI.newOrder).not.toHaveBeenCalled();
            expect(emitted().some(frame => frame.spot_user_data_status?.state === 'reconnecting')).toBe(true);
            await vi.advanceTimersByTimeAsync(5_000);
            expect(moduleMocks.spotPrivateSockets).toHaveLength(2);
            moduleMocks.rendererConnection.sendUTF.mockClear();
            privateFrame(old, { e: 'executionReport', s: 'BTCUSDT', i: 93, X: 'FILLED' });
            old.handlers.close();
            expect(emitted()).toEqual([]);
            await place('after-reconnect');
            expect(moduleMocks.spotClient.restAPI.newOrder).toHaveBeenCalledOnce();
        });

        it('shares one private socket and gives a later renderer the current status', async () => {
            await connectRenderer('spot');
            const handlers = {};
            const second = { ...moduleMocks.rendererConnection, sendUTF: vi.fn(),
                on: vi.fn((name, callback) => { handlers[name] = callback; }) };
            moduleMocks.websocketServerHandlers.request({
                origin: 'http://localhost:5174', accept: () => second,
            });
            await handlers.message({ type: 'utf8', utf8Data: JSON.stringify({ action: 'activate_market', marketMode: 'spot' }) });
            expect(moduleMocks.spotPrivateSockets).toHaveLength(1);
            expect(second.sendUTF.mock.calls.map(([text]) => JSON.parse(text))).toContainEqual(expect.objectContaining({
                generation: 1, spot_user_data_status: expect.objectContaining({ state: 'ready', subscriptionId: 0 }),
            }));
            moduleMocks.rendererHandlers.close();
            expect(moduleMocks.spotPrivateSockets[0].terminate).not.toHaveBeenCalled();
            handlers.close();
            expect(moduleMocks.spotPrivateSockets[0].terminate).toHaveBeenCalledOnce();
            await vi.advanceTimersByTimeAsync(20_000);
            expect(moduleMocks.spotPrivateSockets).toHaveLength(1);
        });

        it('terminates an unacknowledged subscription when the final renderer leaves', async () => {
            moduleMocks.setSpotPrivateAutoAck(false);
            await connectRenderer('spot');
            const socket = moduleMocks.spotPrivateSockets[0];
            moduleMocks.rendererHandlers.close();
        ...
        it('retires the subscription on market switch and restarts it for the new activation', async () => {
            setupBinanceConnection({ localWebSocketAccess: { host: '127.0.0.1' } });
            moduleMocks.websocketServerHandlers.request({ origin: 'http://localhost:5174', accept: () => moduleMocks.rendererConnection });
            await activateSpotRuntime();
            const old = moduleMocks.spotPrivateSockets[0];
            await activateMarket('futures-live');
            ...
            await activateMarket('spot');
            expect(moduleMocks.spotPrivateSockets).toHaveLength(2);
            const current = moduleMocks.spotPrivateSockets[1];
            current.handlers.close();
            expect(emitted().some(frame => frame.generation === 3 && frame.spot_user_data_status?.state === 'reconnecting')).toBe(true);
            moduleMocks.rendererConnection.sendUTF.mockClear();
            privateFrame(old, { e: 'balanceUpdate', a: 'USDT', d: '999' });
            expect(emitted()).toEqual([]);
        });
    });
    ...
    const activateMarket = async (marketMode) => {
        ...
        if (marketMode === 'spot') await vi.advanceTimersByTimeAsync(2_000);
        moduleMocks.rendererConnection.sendUTF.mockClear();
    };
