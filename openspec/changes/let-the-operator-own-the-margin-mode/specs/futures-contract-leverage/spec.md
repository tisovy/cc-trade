...
**Reason**: The requirement names only the multiple, on surfaces that must also name the margin mode: the mode decides whether a loss is bounded by the position's margin or by the wallet, and stating one without the other reads as a complete account of the entry's terms when it is half of one.
**Migration**: Replaced by "The leverage and the margin mode are stated where the position and the order are", which keeps every existing leverage scenario and adds the mode to the ticket and the confirmation panel.

### Requirement: Changing leverage on an open position is stated as risk
**Reason**: Its premise is false, and the desk said it out loud to the operator. The requirement had the control promise that an open position's liquidation price moves with the multiple, and "more plainly" when raising it. The liquidation price this desk computes — and reconciles against the exchange's own figure on every account pass — takes the margin behind the position, the contract's maintenance rate and, in cross, the whole wallet, and no multiple. On 2026-08-21 the operator raised 1× to 2× on a position they were holding, was told its liquidation was moving closer to the mark, and watched nothing move. The requirement also had nothing to say about the one change the exchange refuses outright, `-4161`, which the same operator met minutes later putting the multiple back.
**Migration**: Replaced by "A leverage change on an open position is stated for what it does, and what the exchange refuses is refused here", which keeps the paused-desk rule unchanged, replaces the liquidation claim with what the multiple actually sets, and adds the local refusal.

### Requirement: A flat contract is held at the desk's default of 2× isolated
**Reason**: The margin-mode half of this default overrules a decision the operator made rather than lowering risk they did not choose, and it cannot distinguish an inherited setting from a mode set deliberately in the exchange's own app — which it silently reverted at each restart. The 2× multiple is superseded by 1×.
**Migration**: Replaced by "A flat contract is held at the desk's default of 1×", which keeps the multiple's guards unchanged, lowers the target to 1×, and leaves the margin mode to the operator and the new control.
