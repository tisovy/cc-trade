...
Gross presentation uses the bounded exchange decimal text directly and appends the proven round asset; it never round-trips through a JavaScript `Number` or a fixed two-decimal USDT formatter. Wallet presentation follows the canonical ledger's exact single-asset result even when that sole non-zero asset differs from the round settlement asset. Therefore `+10 realized -1 commission -2 funding` on a USDC round is one exact `+7 USDC` result, while a zero-USDT round whose only wallet movement is `-0.003 BNB` remains an exact `-0.003 BNB` result rather than a relabelled USDT value or a false partial.
