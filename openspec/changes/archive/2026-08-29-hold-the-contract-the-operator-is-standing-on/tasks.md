...
- [x] 2.1 Открыть 龙虾USDT. Тикет должен показывать **LISTING** и причину про
      execution path — не «Select an active USDⓈ-M contract».
- [x] 2.2 `grep 龙虾 ~/.config/cc-trade/diagnostics/desk-<today>-*.jsonl` —
      строки `status` (loading/live/resynchronizing) должны появиться.
- [x] 2.3 Если цикл пересборок повторится — фолты `book-recovery`/`stream` и
      тайминг `aggregate-ready` теперь несут `symbol`: назвать, чья сессия
      штормит (показанная или held из пула).
- [x] 2.4 Оставить деск на 龙虾USDT, перезапустить: деск обязан открыться на
      龙虾USDT, не на предыдущей ASCII-паре.
- [x] 2.5 В журнале при каждом ремаунте: `display` symbol-shown с
      `cause:"restored"`, mounted/unmounted парой, и `link`-строки с
      `connections:1`. `connections:2` при одном окне — второй слушатель,
      сообщить.

Закрыто 2026-08-29 словом оператора («всё работает нормально, я только что
проверил») и журналом desk-2026-08-29-000: тысячи строк по 龙虾USDT (кадры,
11 команд), `display`/`link` пишутся, connections:1, рестарт 07:37:44Z
восстановил последнюю выбранную пару, фолты несут `symbol`. Пункт 2.1
исполнен скрином LISTING 2026-08-28; с `2f73e05` гейт стал стражем
расхождения и в норме не показывается.
