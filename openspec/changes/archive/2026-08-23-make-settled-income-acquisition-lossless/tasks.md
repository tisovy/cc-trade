...
- [x] 3.21 Revalidate confirmation debt when the scheduling debounce fires and preserve confirmation/non-confirmation lane provenance through single-flight follow-up so a reason upgrade cannot leave repaid debt queued for REST
...
- [x] 4.17 Add fast-pass-before-debounce and coalesced-follow-up regressions proving repaid confirmation-only lanes send no request while genuinely new event lanes still run
