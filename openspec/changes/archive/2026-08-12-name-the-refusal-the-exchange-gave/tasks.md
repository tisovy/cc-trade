...
- [x] 4.4 Operator confirms on live data: a deliberately refused order names the exchange's code in the record, and the exchange's message is not in it. — *`verify-the-desk-in-one-sitting/runbook.md`, «Дописано 2026-08-12: отказ называет код биржи», пункт 1* **Confirmed by the operator on 2026-08-12** — a refused placement on APRUSDT landed as `rejected FUTURES_API_ERROR … exchangeCode="-4164"` at 07:40:50Z, and the exchange's message ("notional must be no smaller than 5") is nowhere in the file.
