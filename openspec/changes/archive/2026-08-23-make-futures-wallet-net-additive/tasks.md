...
- [x] 3.1 Render contract/account-shared adjustments once with visible/focusable shared qualifications while keeping compact row-owned coverage detail on its money element without an inline badge
...
- [x] 3.5 Keep one Closed `PnL` column with cents at a glance except where zero would hide a non-zero sub-cent value, and exact/qualified wallet detail in the actual asset on the element, verifying a USDC amount is not passed through a USDT formatter
- [x] 3.20 Render every per-asset open-position settled amount in the compact cell, keep row-owned completeness on the money element, and reserve visible/focusable qualification rows for shared adjustments
...
- [x] 3.32 Render the Closed `PnL` glance value to cents directly from bounded exact realized-PnL text except where zero would hide a non-zero sub-cent value, and retain the exact value/proven asset on the element instead of using `Number` or a USDT-only formatter
...
- [x] 3.34 Preserve exact decimal strings while summing open-position settled totals/components, show cents in the compact cell except when that would hide a non-zero sub-cent amount as zero, retain the exact breakdown on the element, and make no global USDT formatter change
- [x] 3.35 Derive displayed primary and auxiliary open-position totals from their exact owned components so a proven zero survives the ledger's non-zero `visibleNet` projection
...
- [x] 4.20 Add Dock regressions for a settlement-asset total plus auxiliary asset and for a compact partial reading whose coverage remains on the money element without an inline badge
...
- [x] 4.38 Add Closed regressions for cents-at-a-glance, non-zero sub-cent values that must not be hidden as zero, signed-zero, and beyond-safe-integer realized PnL on the element, including non-ready-resource exactness defense
...
- [x] 4.40 Add hook/Dock integration regressions for beyond-safe-integer cents-at-a-glance, non-zero sub-cent preservation, and exact open settled component breakdowns
- [x] 4.41 Add hook/Dock regressions proving cancelling primary and auxiliary components produce a flat exact zero rather than an absent or `null` amount
