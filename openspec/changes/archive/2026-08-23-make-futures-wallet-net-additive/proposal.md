...
- Present shared, multi-asset, and non-USDT adjustment groups visibly and focusably; keep compact row-owned exact/partial detail on the money element without adding a badge or second money column.
...
- Preserve exact decimal strings through open-position settled component sums, round only compact cell text to cents without JavaScript `Number` except where zero would hide a non-zero sub-cent value, and retain the unchanged exact breakdown on the element.
- Preserve an exact zero when owned open-position components cancel, rather than losing the settlement asset through the ledger's intentionally non-zero `visibleNet` projection and presenting a false absence.
