...
- [x] 2.1 После мержа: `[futures-history] … request failed` по 龙虾USDT из
      терминала исчезает; вкладка истории/PnL наполняется сделками пары.
- [x] 2.2 Строки истории по ASCII-парам не изменились (та же выборка, тот же
      порядок).

Закрыто 2026-08-29 словом оператора: ошибок `[futures-history]` в терминале
нет, история пары наполняется, ASCII-пары не изменились (полный прогон
зелёный).
