...
- Enforce bounded urgent overtakes against every skipped queue entry after capacity requeue, not only the current head.
- Make ambiguous account reconciliation report `RESYNCED` only after the complete queued refresh drain proves the mutation's required resources ready.
