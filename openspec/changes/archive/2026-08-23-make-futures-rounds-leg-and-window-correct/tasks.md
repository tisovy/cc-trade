...
- [x] 2.41 Advance the monotonic trade-evidence revision when persisted v2 fills are restored so Closed rounds and wallet reconciliation rebuild before any later REST or stream event
...
- [x] 3.3 Keep one Closed `PnL` column, retain exact realized/wallet detail in the round's proven settlement asset on the element, and verify a USDC round is never labelled USDT
...
- [x] 3.24 Preserve exact exchange realized-PnL text through the Closed `PnL` renderer, round only its glance value to cents without JavaScript `Number` except where zero would hide a non-zero sub-cent value, and retain the exact text on the element
- [x] 3.25 Align the Closed presentation and delta requirement on the operator-approved single `PnL` column without creating standalone `Gross`, `NET`, fee, or funding columns
...
- [x] 4.40 Add Closed renderer regressions proving beyond-safe-integer PnL values show cents at a glance, non-zero sub-cent values are not hidden as zero, and both retain their exact signed decimal and asset on the element
...
- [x] 4.42 Add a Closed header regression proving there is one money column named `PnL` and no standalone `Gross` or `NET` column
...
- [x] 4.44 Add store-to-hook restore regressions proving persisted fills populate Closed Positions immediately while the order-only memoization invariant remains intact
