...
- Version canonical trade evidence independently from order-only history refreshes and retain untouched endpoint collections by reference, so an unrelated order response cannot rematerialize/refold thousands of fills or rebuild every Closed PnL result.
- Advance that trade-evidence revision when canonical persisted fills are restored, so startup cannot reuse the memoized empty Closed index until another exchange event arrives.
- Reconcile the Closed presentation contract around the operator-approved single `PnL` column: cents at a glance except where zero would hide a non-zero sub-cent value, with exact exchange realized PnL, proven asset, and additive exact-or-qualified wallet detail retained on the element instead of separate `Gross`, `NET`, fee, or funding columns.
