...
- [x] 5.2 Operator confirms on live data (gathered as item 2 of the third pass in `verify-the-desk-in-one-sitting/runbook.md`) during a volatile session: a sharp move no longer sends the workspace to `RESYNCHRONIZING`, and any resync that does happen states a cause they can act on.
