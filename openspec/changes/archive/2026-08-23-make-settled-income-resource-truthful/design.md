...
### 12. Keep exact money textual and targets lane-local

Binance income money is an exact JSON string. The HTTP normalizer and dependency-free canonical constructor both reject numeric substitutes, including safe-looking zero, because a number may already have crossed a lossy parser before validation. Aggregate `targetTo` remains the maximum lane target for observation, but `completeByType` and renderer lane admission compare `coveredTo` only with that lane's own non-null target. Resource-wide `complete` additionally requires every lane to cover the aggregate target. This preserves exact older intervals when one narrow trigger advances another lane without making the resource or later intervals falsely complete.
