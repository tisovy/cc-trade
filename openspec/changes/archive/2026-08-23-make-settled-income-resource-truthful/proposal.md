...
- Reject raw numeric income at every canonical boundary because a JavaScript number cannot prove the exchange's original decimal digits.
- Evaluate each `completeByType` flag against that lane's own target so a narrow refresh cannot invalidate unrelated confirmed lanes, while reserving resource-wide completeness for common coverage through the aggregate maximum target.
