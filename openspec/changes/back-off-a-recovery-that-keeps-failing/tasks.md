...
- [ ] 1.1 Add the ceiling to `FUTURES_PRODUCTION_WORKSTATION_BOOK_RECOVERY`
  beside the floor it bounds.
- [ ] 1.2 Count consecutive failed rounds on the session. A round that bridges
  returns the count to zero; a round abandoned mid-flight — contract released,
  session resynchronizing — leaves it where it stood.
- [ ] 1.3 Gate a new round on floor × 2^failures, bounded by the ceiling. The
  `immediate` exemption for buying a page rung stays as it is.

## 2. Prove it bites

- [ ] 2.1 Walk the ladder: after each failed round, a diff arriving one
...
- [ ] 2.2 A bridged snapshot returns the pause to the floor: after a success,
...
- [ ] 2.3 The service suite passes unchanged around it.
