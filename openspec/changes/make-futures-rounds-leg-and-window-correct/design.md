...
The same exact realized-PnL text reaches Closed presentation. The renderer uses it directly for the signed Gross amount and tone, so a sub-cent value cannot become signed zero and a value beyond JavaScript's safe integer range cannot gain or lose units through `Number` conversion.

Closed reconciliation keeps two explicit money columns. `Gross` is the exchange-reported realized amount from the round's fills; `NET` is the canonical additive wallet result when evidence is complete, or a visibly qualified subtotal otherwise. Fee, funding, insurance, credit, asset, and coverage explanations remain on the row/group rather than becoming separate component columns.
