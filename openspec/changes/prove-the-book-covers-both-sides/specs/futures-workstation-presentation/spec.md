...
and SHALL travel with the request that selects the contract so that the first
snapshot of a session is bought at the page that covers it rather than at the
cheapest page and then climbed to. A contract selected without a stated range
SHALL be opened at the largest page the exchange charges its lowest weight for,
and a reading that needs a wider range SHALL take the page that covers it in one
read rather than climbing to it. A range stated for one contract SHALL NOT be
carried to another — it is a distance in the contract's own quote currency, and
...
- **THEN** the first snapshot is bought at the page that covers it, rather than the book opening short and climbing rung by rung
