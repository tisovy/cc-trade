...
- [x] 2.10 Cut the ladder against where the book still has substance rather than against its furthest level. *(Measured 2026-08-14 over five minutes of live stream, the furthest level is an outlier on every contract looked at, and not by a little: AKEUSDT's furthest ask stood at 1 357 378% of price against 145% for the hundredth of the side behind it, and BTCUSDT's furthest bid at 99.0% against 10.05%. Half of AKEUSDT's bids rest inside 11% and half of BTCUSDT's inside 0.8%. Cut against the furthest level, the ladder offers a step whose fourteen rows span a market that is not there — which is the blank far rows this change exists to remove, arriving through the other door. The reading now drops the furthest hundredth of each side. It is expressed as levels dropped rather than as an index, so a short side keeps its whole span: a hundredth of two levels is none of them.)*
- [x] 2.11 Prove by test that one order resting far past the rest of a side does not stretch the reading, and that a side with nothing to spare is still measured to its own edge. *(The first bites against the tree before this change: `expected 999 to be less than or equal to 300`. Both readings are taken off the delivery rather than off an internal method, so what is proved is what the panel is told.)*
