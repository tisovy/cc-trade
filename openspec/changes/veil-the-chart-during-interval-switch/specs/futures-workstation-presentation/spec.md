...
it. While that replacement is in flight, the held chart SHALL appear beneath
a translucent neutral-grey veil with a compact progress indicator above it.
...
- **THEN** only the candles resource reads loading, the held chart is visibly muted beneath the grey progress veil, the book and tape stay live and undisturbed, and the chart replaces its series when the new one arrives
...
- **THEN** the last series stays readable beneath the grey veil with its state and a progress indicator shown until the new one lands, the chart remains interactive, and no panel goes blank
