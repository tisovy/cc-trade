...
import { DrawingProvider } from '../context/DrawingProvider.jsx'
...
function SpotWorkspaceContent() {
  ...
  useEffect(() => {
    ...
    if (currentView === VIEWS.DEPTH && panel?.selected) {
      ...
      // Disable when on MainView or no symbol is selected.
      sendMessage({ action: 'disable_depth_view' });
    }
    return () => {
      if (depthOwned) sendMessage({ action: 'disable_depth_view' });
    };
  ...
  // A command that could not be sent is kept so an explicit resend carries the
  // identity of the first attempt. Rebuilding it would mint a new client order
  // id, and Binance could no longer recognise the two as one intent. The
  // operator-facing resend affordance is added with the Spot failure surface in
  // `report-execution-state-truthfully`; the identity is preserved from now.
    ...
    <>
      ...
      {commandOutcome ? (
        ...
        >
          ...
          <button type="button" onClick={dismissCommandOutcome} aria-label="Dismiss order outcome">
            Dismiss
          </button>
        </div>
      ) : null}
